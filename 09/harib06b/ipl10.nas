...
; TAB=4
		...
		CYLS	EQU	10	; ディスクから10シリンダ分読み込むので10を保存
		ORG 0x7c00	; Where to start the program

; 以下は標準的なFAT12フロッピーディスクのための記述
		...
		TIMES   18 DB 0			; とりあえず18バイト空けておく(nasmでは警告が出るので修正)
...
; ディスクを読む
		...
		MOV		CH,0			; シリンダ0
		MOV		DH,0			; ヘッド0
		MOV		CL,2			; セクタ2 (セクタ1にはipl10.bin(ブートセクタ)が入っているため、OSなどが入っているのはセクタ2から)
readloop:
...
; 読み終わったのでharibote.sysを実行
		...
		MOV		[0x0ff0],CH		; IPLがどこまで読んだのかをメモ(CHはシリンダ → 10シリンダまで読んだことを記録)
		JMP		0xc200
